...
if __name__ == '__main__':
    ...
    with open(r"D:\UCAS\Phd\Projects\201804监测平台\先导专项-分类\地球\train.tsv", "w", encoding="utf-8") as f:
        for i in range(len(train_data)):
            abst = train_data.iloc[i]["文献摘要"].strip()
            if abst:
                f.write(str(num2id[train_data.iloc[i]["文献数据分类"]]) + "\t" + abst + "\n")
    with open(r"D:\UCAS\Phd\Projects\201804监测平台\先导专项-分类\地球\test.tsv", "w", encoding="utf-8") as f:
        for i in range(len(test_data)):
            abst = test_data.iloc[i]["文献摘要"].strip()
            if abst:
                f.write(str(num2id[test_data.iloc[i]["文献数据分类"]]) + "\t" + abst + "\n")
    with open(r"D:\UCAS\Phd\Projects\201804监测平台\先导专项-分类\地球\dev.tsv", "w", encoding="utf-8") as f:
        for i in range(len(test_data)):
            abst = test_data.iloc[i]["文献摘要"].strip()
            if abst:
                f.write(str(num2id[test_data.iloc[i]["文献数据分类"]]) + "\t" + abst + "\n")
